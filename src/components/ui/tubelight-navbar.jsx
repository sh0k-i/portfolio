...
export function NavBar({ items, className }) {
  ...
  return (
    <div
      className={cn(
        "fixed bottom-0 sm:top-0 left-1/2 -translate-x-1/2 z-50 mb-4 sm:pt-0",
        className
      )}
    >
      <div
        className="flex items-center gap-1 bg-dark/50 border border-gray-800 backdrop-blur-lg py-1 px-1 rounded-full shadow-lg hover:border-primary/20 transition-all duration-300"
      >
        {items.map((item) => {
